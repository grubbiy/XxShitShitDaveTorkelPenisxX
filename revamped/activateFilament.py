...
def activateFilament():
    if not wait_for("Tranquil", timeout=50):
        sendStatus("Fant ikke 'Tranquil'")
    click_button("Tranquil")
    pyautogui.rightClick()
    time.sleep(0.4)
    if not wait_for("Use", timeout=50):
        sendStatus("Fant ikke 'Use'")
    click_button("Use")
    time.sleep(0.5)
    if not wait_for("Activate", timeout=50):
